...
# Parameters
Lambda = torch.tensor(1.27e-6, device=device)
plot_accuracy = []
plot_loss = []
...
for t in range(2000):
    ...
    for i in range(10):
        ...
        for param_tensor in state_dict:
            ...
            power = torch.sum(((param_data - glob_data) * h_abs_inverse) ** 2).item()
            transmit_power = transmit_power + power

    Lambda = torch.sqrt(torch.tensor((10 ** (-1.4)) * 10 * 48910/1000)/transmit_power)
    ...
    for i in range(10):
        ...
        for param_tensor in state_dict:
            ...
            h_data = h_gpu[i][param_tensor]
            h_abs_inverse = 1.0 / torch.abs(h_gpu[i][param_tensor])
            power = torch.sum((Lambda * (param_data - glob_data) * h_abs_inverse) ** 2).item()
            Power_Separate_t = Power_Separate_t + power
        Power_Separate[t + 1][i] = Power_Separate_t

    tranPower_toHistory = (transmit_power_history[t] * (t+1) + (Lambda ** 2) * transmit_power/(10*48910/1000))/(t+2)
