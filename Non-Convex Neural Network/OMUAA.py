...
# Parameters
Lambda = torch.tensor(1.75e-6, device=device)
Gamma = torch.tensor(8e-7, device=device)
Qtn = {i: torch.tensor(0.0, device=device) for i in range(10)}
...
for t in range(2000):
    ...
    for i in range(10):
        ...
        for param_tensor in state_dict:
            ...
            h_delay_abs_inverse = 1.0 / torch.abs(h_data)
            g_delay = torch.sum((Lambda * (param_data - glob_data) * h_delay_abs_inverse) ** 2)
            gn = gn + g_delay
            Power_Separate_t = Power_Separate_t + power
        Qtn[i] = torch.maximum(Qtn[i] + (gn - Pnbar), torch.tensor(0.0, device=device))
