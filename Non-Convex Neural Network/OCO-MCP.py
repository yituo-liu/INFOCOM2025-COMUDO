...
# Parameters
Lambda = torch.tensor(1.4e-6, device=device)
GammaMSP = torch.tensor(1e-8, device=device)
EtaMSP = torch.tensor(1e2, device=device)
U = torch.tensor(1e-20, device=device)
Qt = torch.tensor(0.0, device=device)
Gtn = {i: torch.tensor(0.0, device=device) for i in range(10)}
Gtn_next_step = {i: torch.tensor(0.0, device=device) for i in range(10)}
...
for t in range(2000):
    ...
    for i in range(10):
        ...
        for param_tensor, param in local_models[i].named_parameters():
            ...
            param.data = param.data - 0.02 * param.grad - U * Qt * (2 * (Lambda * h_abs_inverse) ** 2)
            power_compute = torch.sum((Lambda * (param.data - glob_data) * h_abs_inverse) ** 2)
            power = power_compute.item()
            transmit_power = transmit_power + power
            gn = gn + power_compute
        weights_local.append(copy.deepcopy(local_models[i].state_dict()))
        Gtn_next_step[i] = gn - Pnbar
        Power_Separate[t + 1][i] = gn
    Qt = torch.maximum(Qt + GammaMSP * (sum(Gtn.values()).to(device) - GammaMSP * EtaMSP * Qt), torch.tensor(0, device=device))
    Gtn = Gtn_next_step
