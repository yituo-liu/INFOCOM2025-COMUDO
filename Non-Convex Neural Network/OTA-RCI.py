...
def FedAvg_Noise(w,Z,Gamma):
    weights_avg = copy.deepcopy(w[0])
    for k in weights_avg.keys():
        for i in range(1, len(w)):
            weights_avg[k] += w[i][k]
        weights_avg[k] += Z[k]
        weights_avg[k] = torch.div(weights_avg[k], len(w) * Gamma)
    return weights_avg
...
# Parameters
Gamma = torch.tensor(1.52e-6, device=device)
Eta = torch.tensor(1e-18, device=device)
U = torch.tensor(3e10, device=device)
Pnbar = torch.tensor((10 ** (-1.4)) / 1000, device=device)
...
for t in range(2000):
    ...
    for i in range(10):
        ...
        for param_tensor, param in local_models[i].named_parameters():
            ...
            h_abs = torch.abs(h_gpu[i][param_tensor])
            Lambda = torch.minimum(Gamma * h_abs / (h_abs ** 2 + Eta), Pnbar * U)
            Coeff = h_abs * Lambda
            param.data = Coeff * param.data
            power = torch.sum((Lambda * (param_data - glob_data)) ** 2).item()
            transmit_power = transmit_power + power
            Power_Separate_t = Power_Separate_t + power
        weights_local.append(copy.deepcopy(local_models[i].state_dict()))
        Power_Separate[t + 1][i] = Power_Separate_t
    ...
    if Adjustment == 1:
        ...
        for i in range(10):
            ...
            for param_tensor, param in local_models[i].named_parameters():
                ...
                glob_data = glob_model.state_dict()[param_tensor]
                h_data = h_gpu[i][param_tensor]
                h_abs_last = torch.abs(h_data)
                Lambda_last = torch.minimum(Gamma * h_abs_last / (h_abs_last ** 2 + Eta), Pnbar * U)
                Coeff_last = h_abs_last * Lambda_last
                param_data = param_data / Coeff_last
                # Update Channel State h
                Hz = torch.sqrt(epsilon_device[i] * (1 - Halpha ** 2) / 2) * torch.randn(size=h_data.shape, dtype=torch.complex64).to(device)
                h_gpu[i][param_tensor] = Halpha * h_data + Hz
                h_abs = torch.abs(h_gpu[i][param_tensor])
    ...
    # Update Global Model
    weights_glob = FedAvg_Noise(weights_local, Z_gpu, Gamma)
    ...
    if (t+1) % 1 == 0:
        ...
        print("整体测试集上的正确率: {}".format(plot_accuracy[t]))
...
plt.figure(2)
plt.plot(transmit_power_history_dBm[0:t+2])
plt.xlabel("time")
